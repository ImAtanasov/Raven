...
public abstract class SwipeHelperAdd extends ItemTouchHelper.SimpleCallback {
    ...
    @SuppressLint("ClickableViewAccessibility")
    public SwipeHelperAdd(Context context, RecyclerView recyclerView, List<Profile> searchList, List<Profile> basicSearchList) {
        super(0, ItemTouchHelper.LEFT);
        this._context = context;
        this.recyclerView = recyclerView;
        this.searchList = searchList;
        this.basicSearchList = basicSearchList;
            ...
            if (dX > 0) {
                ...
                if (dX < 1) {
                    if (is_active != 1) {
                        ...
                        basicSearchList.add(searchList.get(pos));
                        SearchChat sc = (SearchChat)_context;
                        new Handler().postDelayed(() -> sc.closingFloatButtons(), 310);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        _context.startActivity(intent);
                        searchList.get(pos).setLamp(0);
                        sc.addNewInFriendList(searchList.get(pos).getUserID());
